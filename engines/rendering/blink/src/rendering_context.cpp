...
bool RenderingContext::execute_javascript(const std::string& script)
{
    if (!initialized_) {
        return false;
    }
...
std::uint32_t RenderingContext::viewport_width() const
{
    return viewport_width_;
}

std::uint32_t RenderingContext::viewport_height() const
{
...
const RenderingMetrics& RenderingContext::metrics() const
{
    return metrics_;
}
